...
if __name__ == "__main__":
    ...
    job_id        = message.get("jobId", args.job_id)
    configuration = message.get("configuration", {})

    # outputFilePath is generated by the backend as:
    # users/{userId}/simulations/sim_{simulationId}.json
    output_key = message.get("outputFilePath", f"users/unknown/simulations/sim_{job_id}.json")
    ...
    try:
        backend_client.update_simulation_status(job_id, "RUNNING")
    except Exception as exc:
        logger.warning("Could not set RUNNING status: %s", exc)
    ...
    try:
        ...
        try:
            backend_client.update_simulation_status(job_id, "FAILED", error_message=str(exc))
